...
//    }
    ...
    public static boolean boardFull(char[][] board, int rows, int columns) {
    ...
    public static boolean gameWin(char[][] board, int row, int column, char currentPlayer, int rows, int columns) {
        // Check the row
        boolean win = true;
    ...
    public static boolean validMove(char[][] board, int row, int column) {
        return (row >= 0 && row < board.length && column >= 0 && column < board[0].length && board[row][column] == '-');
    }

    public static void createBoard(char[][] board, int rows, int columns) {
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                board[i][j] = '-';
            }
